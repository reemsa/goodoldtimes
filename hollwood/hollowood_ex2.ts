//also found word that has z letter 
const fs = require('fs')
...
class WordFrequencyFramework{
    ...
    register_for_stopword_event(handler) {
        this._z_word_event_handlers.push(handler)
    }
    run(path_to_file) {
        ...
        for (let i = 0; i < this._z_word_event_handlers.length; i++){
            //this._end_event_handlers[i].call(WordFrequencyCounter);
            this._z_word_event_handlers[i]()
        }
    }
}
...
class DataStorage{
    ...
    static _stopword_event_handlers: any = [];
    static _data: any;
    ...
    __produce_words() {
        ...
        for (let w = 0; w < data_str.length; w++){
            if (!DataStorage._stop_word_filter.is_stop_word(data_str[w])) {
                let v = data_str[w].split('')
                if (v.indexOf("z") != -1) {
                    for (let h = 0; h < DataStorage._stopword_event_handlers.length; h++){
                        DataStorage._stopword_event_handlers[h](data_str[w]) 
                    }
                }
    ...
    register_for_z_word_event(handler) {
        DataStorage._stopword_event_handlers.push(handler)
    }
}
...
class WordFrequencyCounter{
    ...
    static _z_word_freqs: any = {};
    constructor(wfapp, data_storage) {
        data_storage.register_for_word_event(this.__increment_count)
        data_storage.register_for_z_word_event(this.__find_z_word)
        wfapp.register_for_end_event(this.__print_freqs)
        wfapp.register_for_stopword_event(this.__z_word)
    }
    __find_z_word(word) {
        
        if (WordFrequencyCounter._z_word_freqs[word] == undefined) {
            WordFrequencyCounter._z_word_freqs[word] = 1 
        }
        else {
            WordFrequencyCounter._z_word_freqs[word] += 1
        }
    ...
    __z_word() {
        let data = WordFrequencyCounter._z_word_freqs
        let arraystop:any=[]
        Object.keys(data).sort(function (a, b) { return data[b] - data[a] })
    .forEach(function (key) { arraystop.push(`${key},${data[key]}`) });
    for (let i = 0; i < arraystop.length; i++){
        console.log("word_with_z = "+arraystop[i])
    }
    }
