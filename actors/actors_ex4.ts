...
class DataStorageManager extends ActiveWFObject{
  ...
  constructor() {
    ...
    init(file_path:string,swm:StopWordManager){
        this.data=fs.readFileSync(file_path,"utf8").toLowerCase().replace(/[^A-Za-z0-9\n]+/g, " ")
        this.stop_word_manager=swm;
    }
    process_words(message):any{
        this.recipient=message;
        let words:any=this.data.split("\n");
        for (let i = 0; i < words.length; i++){
            words[i]=words[i].split(" ")
            for (let j = 0; j < words[i].length; j++){
                send (this.stop_word_manager,['filter',[words[i][j],String(Number(Math.floor(i / 10) + 1))]])  
            }
        }
...
class StopWordManager extends ActiveWFObject{
  ...
  constructor() {
    ...
    filter(word:string){
        if (this.stop_words.indexOf(word[0])==-1){
...
class WordFrequencyManager extends ActiveWFObject{
  ...
  constructor() {
    ...
    increment_count(word) {
        if (this.word_freq[word[0]] != undefined) {
            this.word_freq[word[0]][1] += 1
            if (this.word_freq[word[0]][2].split("-").indexOf(word[1]) == -1) {
                this.word_freq[word[0]][2] = this.word_freq[word[0]][2] + "-" + word[1]
            }
            
        }
        else {
            this.word_freq[word[0]]=[word[0],1,word[1]] 
        }
    }
  top25(re) {
  ...
  constructor() {
    ...
    display(wf) {
        for (let i=0;i<wf.length;i++){
            ...
            if (wf[i].split(",")[1] < 25) {
                console.log("data = "+wf[i].split(',')[0]+":"+wf[i].split(',')[2])
            }
        }
...
send(storage_manager,['init',"inputFile.txt",stop_word_manager])
