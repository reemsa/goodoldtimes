...
class master{
    ...
    constructor(file_name) {
       master.file_path=file_name
    }
    bind(func) {
        try {
          master.file_path=func(master.file_path)
        } 
        catch (error) {
            console.log(error)
      }  
...
let c: master = new master(file_name)
c.bind(extract_words)
c.bind(remove_stop_words)
c.bind(frequencies)
c.bind(sort)
c.bind(print)
