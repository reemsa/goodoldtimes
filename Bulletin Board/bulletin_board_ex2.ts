...
class StopWordFilter{
    ...
    is_stop_word(event) {
        ...
        if ((StopWordFilter._stop_words.indexOf(word) == -1) && (word.trim() != '')) {
            if (word.split("").indexOf('z') != -1) {
                StopWordFilter._event_manager.publish(['z_word',word])   
            }
...
class WordFrequencyCounter {
    ...
    print_freqs_z(event) {
        ...
        for (let i = 0; i < array.length; i++) {
            console.log("word_with_z = " + array[i])
        }  
    }
...
class WordFrequencyApplication{
    ...
    stop(event) {
        ...
        WordFrequencyApplication._event_manager.publish(['print_z', null])
    }
}
