...
let remove_stop_words = (word_stop_list) => {
    function _f() {
        let word_list=word_stop_list[0].replace(/[^A-Za-z0-9]+/g, " ").toLowerCase().split(' ')
        let stop_words=word_stop_list[1].toLowerCase().split(',')
        let array=[]
